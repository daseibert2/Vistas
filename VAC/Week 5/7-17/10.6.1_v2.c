...
double exact(double t,double x, double n_initial)
{
...
void printAll(double u[30][30],int t_size,int x_size,double dt, double dx,double n_initial)
{
    double ex=0,err=0;
...
void zeroesU(double u[30][30],double dx,int t_size,int x_size,double n_initial)
{
...
void boundaries(double u[30][30],int row, int x_size)
{
    u[row][0]=0;
    u[row][x_size-1]=0;
}
...
void fill(double u[30][30],int row, double Y, int x_size, double dt,double dx)                          //Explicit
{
...
void Explicit(double dx,double Y,double n_initial,int x_size,double dt,int t_size)
{
    printf("\nPart 1: Explicit\n\n");

    double u[t_size][x_size];

    zeroesU(u,dx,t_size,x_size,n_initial);
    printf("zeroes\n");
    printf("%d %d ",t_size,x_size);
    for(int n=1;n<t_size;n++)
    {
        //boundaries(u,n,x_size);
        fill(u,n,Y,x_size,dt,dx);
        printf("fill ");
    }

    printAll(u,t_size,x_size,dt,dx,n_initial);
...
//-----------------------------------------Part 1: Explicit---------------------------------------------
    ...
    Explicit(dx,Y,n_initial,x_size,dt,t_size);
